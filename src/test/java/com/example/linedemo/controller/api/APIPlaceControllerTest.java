...
import com.example.linedemo.dto.request.PlaceRequest;
...
class APIPlaceControllerTest {
    ...
    @Test
    void givenPlace_whenCreate_thenReturnSuccessfulResponse() throws Exception{
        //Given
        PlaceRequest placeRequest = PlaceRequest.of(
                null,
                PlaceType.COMMON,
                "우리집",
                "광주 광역시 광산구 목련로 153번길 56",
                "01033089531",
                30,
                "신장개업"
        );
    ...
    @DisplayName("(API)[GET] 단일 장소 조회 - 장소가 있는 경우, 장소 데이터를 담은 표준 API 출력")
    @Test
    void givenPlaceId_whenRequestingPlace_thenReturnsPlace() throws Exception {
        ...
        mvc.perform(get("/api/places/"+placeId))
                ...
                .andExpect(jsonPath("$.data.placeName").value("우리집"))
                .andExpect(jsonPath("$.data.address").value("광주 광역시 광산구 목련로 153번길 56"))
                .andExpect(jsonPath("$.data.phoneNumber").value("01033089531"))
    ...
    @Test
    void givenPlaceId_whenNullPlace_thenBlankResponse() throws Exception{
        // Given
        long placeId = 2L;

        //When&Then
        mvc.perform(post("/api/places"+placeId))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    void givenPlace_whenModify_thenResponseSuccess() throws Exception{
        //Given
        long placeId = 1L;
        PlaceRequest placeRequest = PlaceRequest.of(
                null,
    ...
    @Test
    void givenPlaceId_whenDelete_thenResponseSuccess() throws Exception {
        long placeId = 1L;
